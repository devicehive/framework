(function ($, window) {
    ...
    ajax = function (self, method, url, params) {
    	return $.ajax({
    		...
    		beforeSend: function (jqXnr) {
    			jqXnr.setRequestHeader("Authorization", "Basic " + encodeBase64(self.login + ":" + self.password));
    		}
    	});
    },
	...
	var deviceHive = function (serviceUrl, login, password) {
	...
	deviceHive.prototype = {
		...
		setDevice: function(deviceId, params){
			return query(this, "PUT", "/device/" + deviceId, params);
		},
		...
		getNotifications: function (deviceId, start, end) {
			...
			return query(this, "GET", "/device/" + deviceId + "/notification", params);
		},
		...
		openChannel: function (channels) {
			...
			var deferred = infoQuery.pipe(function (info) {
				...
				var dfd = $.Deferred().reject("DeviceHive: None of the specified channels are supported");
				$.each(channels, function () { // enumerate all channels in order
					var channel = this;
					if (deviceHive.channels[channel]) {
						dfd = dfd.pipe(null, function () { // in case of failure - try next channel
	...
	deviceHive.channels.webSocket.prototype = {
		...
		sendCommand: function (deviceId, command, parameters) {
			...
			request.result = function (callback) {
				$(request).bind("onResult", function (e, thisCommand) {
					callback.call(self.hive, thisCommand);
				});
			};
			return request;
		},
